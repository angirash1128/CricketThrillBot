...
def get_bottom_menu():
    # is_persistent fix - was 'persistent' before (wrong)
...
def send_sos_alert(user_id, message_text):
    ...
    keyboard.add(types.InlineKeyboardButton(
        "✅ I am Watching", callback_data="sos_watching"))
    try:
...
def start_sos_for_all(alert_message):
    ...
    for user in users:
        uid = user["user_id"]
        sos_state[uid] = {"message": alert_message, "count": 0, "active": True}
...
def handle_start(message):
    ...
    keyboard.row(types.InlineKeyboardButton(
        "🔔 Set Up My Alerts", callback_data="setup_start"))
...
def setup_start(call):
    ...
    buttons = [types.InlineKeyboardButton(
        team, callback_data=f"team_{i}") for i, team in enumerate(IPL_TEAMS)]
...
def handle_subscribe_match(call):
    ...
    bot.send_message(
        ...
        reply_markup=get_bottom_menu(), parse_mode="HTML"
    )
...
def handle_feedback_text(message):
    ...
    bot.send_message(
        ...
        "Your response has been saved. 🙏" if saved else
...
def show_settings(user_id, chat_id):
    ...
    keyboard.row(types.InlineKeyboardButton(notif_btn, callback_data=notif_cb))
    keyboard.row(types.InlineKeyboardButton(
        "🏠 Main Menu", callback_data="main_menu"))
...
def change_team(call):
    ...
    keyboard = types.InlineKeyboardMarkup(row_width=2)
    buttons = [types.InlineKeyboardButton(
        team, callback_data=f"newteam_{i}") for i, team in enumerate(IPL_TEAMS)]
    keyboard.add(*buttons)
...
if __name__ == "__main__":
    ...
    # Start polling - skip_pending ignore karta hai purane messages
